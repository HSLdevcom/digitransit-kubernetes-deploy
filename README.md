...
- Run script _start-ansible-shell.sh_
...
- _For interacting with AKS_: When you start the management container, install kubectl and connect to the cluster as follows:
...
- Use folder _group_vars_ for variables that do not vary between deployments or environments, such as the naming convention of resources

  - In folder _group_vars_, use file _all_ for generic variables (e.g. Resource Group namings, variables related to subscription etc.)

- Use folder _env_vars_ for variables that DO vary between deployments or environments, such as the number of nodes in an AKS cluster
